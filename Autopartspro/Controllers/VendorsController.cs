...
public class VendorsController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<VendorDto>>> List([FromQuery] string? search)
    {
        ...
        var items = await q.OrderBy(v => v.Name)
            .Select(v => new VendorDto(v.Id, v.Name, v.ContactPerson, v.Email, v.Phone, v.Address, v.CreatedAt, v.UpdatedAt))
            .ToListAsync();
        return Ok(items);
    }
    ...
    public async Task<ActionResult<VendorDto>> Get(int id)
    {
        ...
        return new VendorDto(v.Id, v.Name, v.ContactPerson, v.Email, v.Phone, v.Address, v.CreatedAt, v.UpdatedAt);
    }
    ...
    public async Task<ActionResult<VendorDto>> Create([FromBody] VendorUpsertDto dto)
    {
        ...
        var result = new VendorDto(v.Id, v.Name, v.ContactPerson, v.Email, v.Phone, v.Address, v.CreatedAt, v.UpdatedAt);
        return CreatedAtAction(nameof(Get), new { id = v.Id }, result);
    }

    [HttpPut("{id:int}")]
    public async Task<ActionResult<VendorDto>> Update(int id, [FromBody] VendorUpsertDto dto)
    {
        var v = await _db.Vendors.FindAsync(id);
        if (v is null) return NotFound();
